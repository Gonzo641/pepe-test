...
import { resetCart } from "@/redux/foodSlice";
import PaymentForm from "@/components/PaymentForm";
import Link from "next/link";

const CartPage = () => {
  const { productData } = useSelector((state: StateProps) => state?.food);
    ...
    <Container>
      {productData.length > 0 ? (
        ...
        <div className="flex flex-col gap-y-6 items-center justify-center bg-white h-96 px-4">
          <p className="border-[1px] border-red-600 w-full p-2 text-center">
            Your product cart is currently empty
          </p>
          <Link href={"/"}>
            <button className="bg-darkText text-white py-2 px-6 rounded-md hover:bg-red-600 duration-200">
              Return to Shop
            </button>
          </Link>
        </div>

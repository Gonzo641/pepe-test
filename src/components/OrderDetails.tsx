...
import { resetOrder } from "@/redux/foodSlice";
import Link from "next/link";

const OrderDetails = () => {
  const dispatch = useDispatch();
  const { orderData } = useSelector((state: StateProps) => state?.food);
      ...
      {orderData?.order?.length > 0 ? (
        <div>
          ...
          <button
            ...
            className="mt-5 border-[1px] border-gray-500 py-1 px-4 font-medium rounded-md 
                    hover:border-red-600 cursor-pointer duration-200"
          >
            Reset Order
          </button>
        </div>
        ...
        <div className="py-10 bg-white text-black text-2xl text-center">
          <p>Nothing to show</p>
          <Link href={"/"}>
            <button className="bg-black text-slate-100 w-44 h-12 rounded-full 
